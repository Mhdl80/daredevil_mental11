...
if matches[1] == 'sticker' then
      ...
      if matches[2] == 'warn' then
        ...
        return 'حالت اخطار قفل استیکر فعال شد!\nارسال کننده استیکر برای بار اول اخطار دریافت میکند،\nو در صورت ارسال مجدد استیکر اخراج خواهد شد.'
      end
      if matches[2] == 'kick' then
        ...
        return 'قفل استیکر فعال شد و ارسال کننده استیکر بدون تذکر اخراج خواهد شد!'
      end
    ...
    if msg.media and msg.media.caption == 'sticker.webp' and not is_momod(msg) then
      ...
      if settings.sticker == 'warn' then
        if is_sticker_offender then
          ...
          return 'شما قبلاً اخطار دریافت کرده بودید\nو بخاطر ارسال مجدد استیکر اخراج میشوید،بای😐✋'
        elseif not is_sticker_offender then
          redis:set(sticker_hash, true)
          return 'لطفا استیکر ارسال نکنید.\nدر صورت ارسال مجدد از گفتگو در این گروه محروم خواهید شد😐!'
        end
      elseif settings.sticker == 'kick' then
        chat_del_user(receiver, 'user#id'..user_id, ok_cb, true)
        return 'شما بخاطر ارسال استیکر اخراج میشوید،بای😐✋'
      elseif settings.sticker == 'ok' then
        return nil
      end
    end
